-- Fix for hybrid_search_sessions: Ensure it returns results even with low keyword scores
-- The issue: keyword_score might be very small (near 0) but still should count as a match
-- This version ensures that if EITHER semantic OR keyword matches, the result is returned
...
BEGIN
    ...
    WHERE 
        ...
        -- IMPORTANT: Use OR condition - if EITHER matches, include the result
        AND (
            -- Semantic match: similarity above threshold
            (1 - (s.embedding <=> query_embedding)) >= match_threshold OR
            -- Keyword match: full-text search finds the query text
...
COMMENT ON FUNCTION hybrid_search_sessions IS 'Hybrid search combining semantic (vector) and keyword (full-text) search with metadata filtering. Returns results ranked by combined score. Uses OR logic: returns results if EITHER semantic OR keyword matches.';
